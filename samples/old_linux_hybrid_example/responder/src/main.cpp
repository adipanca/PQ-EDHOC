...
hnd_post(coap_resource_t *resource, coap_session_t *session, const coap_pdu_t *request,
	...
	if(rx_msg_num == 1){
		...
		PRINT_MSG("coap response code changed before add data large\n");
		...
		PRINT_MSG("Waiting for the MSG3 .........\n");							
	}else{
...
enum err tx(void* sock,struct byte_array *data)
{
   ...
   PRINT_ARRAY("my baffer",my_buffer2,my_buffer_len2);
   sem_post(&semaphore);
   PRINT_MSG("sem posted\n");
   return ok;
}
...
void * edhoc_responder_init(void *arg)
{
	...
	get_suite((enum suite_label)c_r.suites_r.ptr[c_r.suites_r.len - 1],
		      &suit_in);
	//uint8_t G_Y[get_kem_cc_len(suit_in.edhoc_ecdh)];
	uint8_t GY[get_kem_cc_len(suit_in.edhoc_ecdh)+get_ecdh_pk_len(suit_in.edhoc_ecdh)];
	c_r.g_y.ptr = GY;
	...
	byte_array G_Y_random;
	G_Y_random.ptr = GY;
	G_Y_random.len = get_kem_cc_len(suit_in.edhoc_ecdh)+get_ecdh_pk_len(suit_in.edhoc_ecdh);
